      ...
      <section id="prezzi" style={{ background: BG, padding: "5rem 1.5rem", borderBottom: `1px solid ${BD}` }}>
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {packages.map((p, i) => (
              <div key={i} className="relative p-6 flex flex-col bg-white" style={{
                border: p.highlight ? `2px solid ${AC}` : `1px solid ${BD}`,
                boxShadow: p.highlight ? `0 0 24px ${AC}20` : "none",
              }}>
                {p.badge && (
                  <div style={{ position: "absolute", top: -1, right: 16, background: AC, color: W, fontFamily: MONO, fontWeight: 700, fontSize: "0.58rem", letterSpacing: "1px", textTransform: "uppercase", padding: "3px 10px" }}>
                    {p.badge}
                  </div>
                )}
                <p style={{ fontFamily: MONO, fontWeight: 700, color: GR, fontSize: "0.62rem", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "0.75rem" }}>{p.name}</p>
                <p style={{ fontFamily: MONO, fontWeight: 700, color: p.highlight ? AC : P, fontSize: "2.2rem", lineHeight: 1, marginBottom: "0.25rem" }}>{p.price}</p>
                <p style={{ fontFamily: SERIF, color: GR, fontSize: "0.78rem", marginBottom: "1.5rem" }}>{p.desc} — {p.credits}</p>
                <ul className="space-y-2 mb-6 flex-1">
                  {p.features.map((f, j) => (
                    <li key={j} className="flex items-center gap-2">
                      <CheckCircle2 style={{ width: 13, height: 13, color: AC, flexShrink: 0 }} />
                      <span style={{ fontFamily: MONO, fontSize: "0.7rem", color: TX }}>{f}</span>
                    </li>
                  ))}
                </ul>
                <button onClick={handleLogin} style={{
                  background: p.highlight ? AC : "transparent",
                  color: p.highlight ? W : AC,
                  border: `1px solid ${AC}`,
