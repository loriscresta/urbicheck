...
Deno.serve(async (req) => {
  ...
  let parcel = null;
  try {
    const res = await fetch(lookupUrl, { headers: { 'User-Agent': 'UrbiCheck/1.0 (info@urbicheck.it)', 'Accept': 'application/json', 'X-Api-Key': Deno.env.get('CATASTO_API_KEY') }, signal: AbortSignal.timeout(10000) });
    if (res.ok) {
  ...
  if (!parcel) {
    ...
    if (comuneLat && comuneLon) {
      ...
      try {
        const res = await fetch(fallbackUrl, { headers: { 'User-Agent': 'UrbiCheck/1.0 (info@urbicheck.it)', 'Accept': 'application/json', 'X-Api-Key': Deno.env.get('CATASTO_API_KEY') }, signal: AbortSignal.timeout(10000) });
        if (res.ok) {
          const data = await res.json();
          if (data.found && data.parcels?.length > 0) {
            parcel = data.parcels[0];
